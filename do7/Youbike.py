...
def appendDistance(lat, lng, youbikes):
    for youbouke in youbikes:
        d = haversine(lat, lng, float(youbouke.get("lat")), float(youboke.get("lng")))
        youboke.setdefau("distance", d)
...
def getYouBike(sna, youbikes=None) -> dict:
    if youbikes is None:
